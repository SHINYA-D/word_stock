...
def main() -> int:
    ...
    h = box(MAIN_X, y, MAIN_W, "user", "👤 ユーザー入力",
            ["「テストケースを作成して」とチャットに入力",
             "一部のファイルだけを依頼することもできる（→ 手順0 の範囲宣言）"])
    ...
    # ══ 2. 手順0: 範囲宣言 ══
    sy = y
    h = box(MAIN_X, y, MAIN_W, "main", "手順0: 範囲宣言（部分依頼のときだけ）",
    ...
    # ══ 個別ループ枠 ══
    loop_top = y - 40
    h = box(MAIN_X, y + 52, MAIN_W, "main", "計画（手順1）",
    ...
    step2_y = y             # continue 差し戻しの戻り先（手順2）
    hy = y
    h = box(MAIN_X, y, MAIN_W, "main", "手順2: Agent(サブエージェント起動)",
    ...
    h = box(MAIN_X, y, MAIN_W, "agent",
            "🟩 SUBAGENT: test-unit / test-widget-test-generator",
    ...
    hy = y
    h = box(MAIN_X, y, MAIN_W, "agent", "サブエージェント自身によるハーネス実行",
    ...
    bh = box(LEFT_X, dy, LEFT_W, "bug", "🐞 プロダクションコードのバグ",
             ["コードは直さず production_bugs に記録",
              "skipped にして次の対象へ（止めない）"])
    connect(LEFT_X + LEFT_W, dy + bh / 2, MAIN_X, dy + 25)
    # continue の差し戻し（手順2 のエージェント起動へ戻る）
    ...
    text(MAIN_X - 26, (dy + step2_y) / 2 + 8, "手順2〜4 を反復",
         11, "#b91c1c", bold=True, anchor="end")
    arrow(cx, y + h, y + h + 34, "#047857", "stop", "#047857"); y += h + 34

    # ── 記録（手順6）──
    ry = y
    h = box(MAIN_X, y, MAIN_W, "main", "記録（手順6）",
    ...
    hy = y
    h = box(MAIN_X, y, MAIN_W, "script", "⑦ 全体回帰",
    ...
    hy = y
    h = box(MAIN_X, y, MAIN_W, "agent", "⑧ 🟩 SUBAGENT: architecture-guard",
    ...
    h = box(MAIN_X, y, MAIN_W, "main", "⑨ 網羅性・必要性の自己監査（メインが実施）",
            ["正常系/異常系/境界値の欠落・重複・無価値テスト・対象外理由の妥当性を点検",
             "指摘は該当サブエージェントへ差し戻して修正"])
    arrow(cx, y + h, y + h + 30); y += h + 30

    hy = y
    h = box(MAIN_X, y, MAIN_W, "agent", "⑩ 🟩 SUBAGENT: test-doc-excel-generator",
    ...
    fh = box(LEFT_X, ey, LEFT_W, "file", "最終成果物",
             ["~/Desktop/",
              ...
              "（要確認一覧: 理由なし未達 / バグ /",
              " テスト失敗 / テスト漏れ は赤字）"], mono_lines=True)
    connect(LEFT_X + LEFT_W, ey + fh / 2, MAIN_X, ey + 25)
    arrow(cx, y + h, y + h + 30); y += h + 30

    hy = y
    h = box(MAIN_X, y, MAIN_W, "main", "⑪ セッション破棄",
    ...
    box(RIGHT_X, hy, RIGHT_W, "stop", "🚧 end-session 側の関所（loop_state.py）",
        ["completed_at なし → 非 0 で拒否（先に手順10 を実行させる）",
    ...
    h = band(LEFT_X, stop_y, W - 40, "stop",
             ...
             ["解除条件 …… ① .test_loop/state.json が無い（テスト工程外）　"
              ...
              "　　　　　　　  ・verdict が stop なのに finish 未実行 → 手順6 を促す",
              "　　　　　　　  ・scope（未設定なら全対象）に未消化が残っている → 手順1 へ戻す",
              "　　　　　　　  ・全対象 done だが completed_at 無し → 手順7〜11 を促す",
